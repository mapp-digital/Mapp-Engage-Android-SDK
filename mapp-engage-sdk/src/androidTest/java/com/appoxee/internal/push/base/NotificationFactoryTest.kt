...
class NotificationFactoryTest {
    ...
    fun setUp() {
        ...
        iconProvider = mockk(relaxed = true) {
            every { getLargeIcon() } returns mockk<Bitmap>(relaxed = true)
            every { getSmallIcon() } returns 0
            every { getSmallIconApi23() } returns mockk()
        }
        ...
        notificationStyleFactory = mockk(relaxed = true) {
            every { buildNotificationStyle(any()) } returns mockk(relaxed = true) {
                coEvery { getStyle() } coAnswers { NotificationCompat.BigTextStyle() }
            }
        }
    ...
    fun testCreateSimpleNotification() = runBlocking {
        ...
        verify(exactly = 1) { builder.setPriority(NotificationCompat.PRIORITY_HIGH) }
