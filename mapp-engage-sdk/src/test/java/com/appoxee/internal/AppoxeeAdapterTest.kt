...
class AppoxeeAdapterTest {
    ...
    fun `inappEvent with successful response`() {
        runTest {
            ...
            val response = appoxeeAdapter.inappEvent(
                "1", 101, TrackingKey.IA_MSG_DISPLAYED,
                emptyMap()
            )
            coVerify { engageApi.inappEvent(any(), any(), any(), any()) }
            Truth.assertThat(response.statusCode).isEqualTo(200)
        }
    }
    ...
    fun `inappEvent with error response`() {
        runTest {
            ...
            coVerify { engageApi.inappEvent(any(), any(), any(), any()) }
            Truth.assertThat(response.error).isInstanceOf(SecurityException::class.java)
        }
    }
    ...
    fun `pushEvent with successful response`() {
        runTest {
            ...
            val response = appoxeeAdapter.pushEvent(
                1234, 101, ClickType.OPEN_LANDING_PAGE,
                EventType.CLICK
            )
            coVerify { engageApi.pushEvent(any(), any(), any(), any()) }
